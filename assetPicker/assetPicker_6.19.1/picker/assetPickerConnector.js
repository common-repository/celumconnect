...
Celum.AssetPickerConnector = function () {
  ...
  var applyScript = function (index) {
    if (index < USED_SCRIPTS.length) {
      ...
      script.setAttribute('src', USED_SCRIPTS[index] + '?v=' + new Date().getTime());
  ...
  return {
    ...
    applyCss: function () {
      ...
      if (css !== null) {
        ...
        style.setAttribute('href', css + '?v=' + new Date().getTime());
    ...
    // TODO: remove when everything is migrated to REST API (maybe)
    getLocaleValue: function (values, currentLocale) {
      var localeValue = undefined;
      var defaultValue = undefined;

      // TODO: remove this part when everything is migrated to REST API
      angular.forEach(values, function (name) {
        if (name.locale == currentLocale) {
          localeValue = name.value;
        }
        if (name.locale == Celum.AssetPickerConnector.getDefaultLocale()) {
          defaultValue = name.value;
        }
      });

      if (values[currentLocale]) {
        localeValue = values[currentLocale]
      }
      if (values[this.getDefaultLocale()]) {
        defaultValue = values[this.getDefaultLocale()]
      }

      //Display default name if there is no translation
      if (!angular.isDefined(localeValue) && angular.isDefined(values)) {
        // keep the old fallback to just the first entry to accommodate for old configurations without defaultLocale property
    ...
    getLocaleValueRestAPI: function (values, currentLocale) {
      ...
      if (values[currentLocale] !== undefined) {
        localeValue = values[currentLocale];
      }

      defaultValue = values[Celum.AssetPickerConnector.getDefaultLocale()];

      //Display default name if there is no translation
      if (!angular.isDefined(localeValue) && angular.isDefined(values)) {
        // keep the old fallback to just the first entry to accommodate for old configurations without defaultLocale property
        localeValue = defaultValue || values[0].value;
      }
    ...
    handleMessageEvents: function (event) {
      ...
      if (parentOrigin === null || event.origin === parentOrigin) {
        ...
        if (parentOrigin === null || util.getFrameProperty('id') == data.target) {
          if (typeof data.action === 'string' && data.action !== 'languageChanged') {
