...
Celum.AssetPicker = (function () {
  ...
  var util = function () {
    return {
      ...
      pathPrefix: 'wp-content/plugins/'+plugin_data.plugin_basename+'/assetPicker/assetPicker_6.15.12/picker/'
    }
  }();
  // @formatter:on
  ...
  var frames = function () {
    ...
    return {
      ...
      allowCrossDomain: function () {
        console.warn('You are allowing to send data for an arbitrary origin to enable cross domain usage - is this really your intention?');
      ...
      applyListeners: function (frame, listeners) {
        ...
        var handleListeners = function (frame, event, listeners) {
          if (allowCrossDomain || event.origin === util.getValidOrigin()) {
            ...
            // note that multiple selectors currently only work SAME domain!
            if (!allowCrossDomain && data.id !== frame.id) {
              return;
            }
